...
class MyError extends Error {
  constructor(public readonly message: string = "MyError") {
    super(message);
  }
}

describe("assert는", () => {
  test('"condition"을 단언할 수 있다.', () => {
  ...
  test('"condition"을 만족하지 않는 경우 오류를 발생시킨다.', () => {
    // eslint-disable-next-line @typescript-eslint/no-inferrable-types
    const value: string = "string";

    expect(() => assert(value === "not_string")).toThrowError(Error);
    expect(() =>
      assert(value === "not_string", new MyError("잘못된 문자열"))
